...
def move_piece(turn, start_coords, end_coords, board, piece, pieces_arr):
    """
    ...
    Wking_origin = [7, 4]
    Bking_origin = [0, 4]
    if piece.__class__.__name__ == "King" and end_coords[1] == start_coords[1] - 2 or end_coords[1] == start_coords[1] + 2:
        develop_castle(turn, board, pieces_arr)
    else:# Mark captured piece as dead
        dead_arr = []
        for p in pieces_arr:
...
def develop_castle(turn, board, pieces_arr):
    '''
    Rules:
    ...
    if not king_checked(board, king, pieces_arr):
        if turn == "White":
            if (board[white_king[0]][white_king[1]].__class__.__name__ == "King" and board[white_king[0]][white_king[1]].color == turn) and (board[white_rook1[0]][white_rook1[1]].__class__.__name__ == "Rook" and board[white_rook1[0]][white_rook1[1]].color == turn) or (board[white_rook2[0]][white_rook2[1]].__class__.__name__ == "Rook" and board[white_rook2[0]][white_rook2[1]].color == turn):
        ...
        if turn == "Black":
            if (board[black_king[0]][black_king[1]].__class__.__name__ == "King" and board[black_king[0]][black_king[1]].color == turn) and (board[black_rook1[0]][black_rook1[1]].__class__.__name__ == "Rook" and board[black_rook1[0]][black_rook1[1]].color == turn) or (board[black_rook2[0]][black_rook2[1]].__class__.__name__ == "Rook" and board[black_rook2[0]][black_rook2[1]].color == turn):
                #Run simulation to see if king can reach two spaces left or right without getting attacked.
                orig_king = board[0][4]
